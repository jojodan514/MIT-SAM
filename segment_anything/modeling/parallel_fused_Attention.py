...
class parallel_fused_AttentionBlock(nn.Module):
    ...
    def forward(
        ...
        resudal_origin = image
        # self attention for each other

        # print('lang.shape',lang.shape)
        # print('langpe.shape',langpe.shape)
        ...
        image = resudal + image + resudal_origin
...
class parallel_fused_Attention(nn.Module):
    def __init__(
        ...
        self.self_attn_image_last = Attention(embedding_dim, num_heads)
        self.norm_image_last = nn.LayerNorm(embedding_dim)
    ...
    def forward(
        ...
        image_pe = image + image_pe
        lang_pe = lang + point_embedding
        attn_out = self.self_attn_image_last(q=image_pe, k=lang_pe, v=lang)
        image = image + attn_out
        image = self.norm_image_last(image)
