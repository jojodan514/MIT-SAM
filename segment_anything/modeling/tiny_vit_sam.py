...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x, lang):
        # x: (N, C, H, W)
        process_x_list = []
        x = self.patch_embed(x)
        # print('x.shape after patch', x.shape)

        x = self.layers[0](x)
        start_i = 1
        # print('x.shape after convlayer', x.shape)


        for i in range(start_i, len(self.layers)):
            layer = self.layers[i]
            x = layer(x)
            process_x_list.append(x)
            # print('x.shape after basic layer {}'.format(i), x.shape)
        B,_,C=x.size()
        x = x.view(B, 64, 64, C)
        x=x.permute(0, 3, 1, 2)
        # print('x.shape after all layer', x.shape)
        x=self.neck(x)
        # print('x.shape after neck', x.shape)
        return x, process_x_list

    def forward(self, x, lang):
        x = self.forward_features(x, lang)
        #x = self.norm_head(x)
        #x = self.head(x)
        return x
...
if __name__ == '__main__':
    x = torch.rand((8,1024,256))
    # tiny_vit = TinyViT(img_size=1024, in_chans=3, num_classes=1000,
    #             embed_dims=[64, 128, 160, 320],
    #             depths=[2, 2, 6, 2],
    #             num_heads=[2, 4, 5, 10],
    #             window_sizes=[7, 7, 14, 7],
    ...
    basic_layer = BasicLayer(dim=256, input_resolution=[32,32], depth=2, num_heads=8, window_size=7)
    y = basic_layer(x)
